...
export default function RfisPanel({ projectId }: { projectId: string }) {
  ...
  async function saveEditing(rfiId: string) {
    ...
    const { error } = await supabase
      .from("rfis")
      .update({ subject: editSubject, question: editQuestion })
      .eq("id", rfiId);

    if (!error) {
      setRfis((prev) =>
        prev.map((r) => (r.id === rfiId ? { ...r, subject: editSubject, question: editQuestion } : r))
      );
      setEditingId(null);
    }
    setSaving(false);
  ...
  if (loading) {
    return <p className="text-graphite-500 text-xs">Cargando RFIs...</p>;
  }
    ...
    return (
      <div className="bg-graphite-900 border border-dashed border-graphite-600 rounded-lg p-8 text-center">
        ...
        <p className="text-graphite-400 text-sm">
          Todavía no generaste ningún RFI. Podés crear uno desde un hallazgo.
        </p>
      </div>
    );
  }
          ...
          <div key={rfi.id} className="bg-graphite-900 border border-graphite-700 rounded-lg overflow-hidden">
            ...
            >
              ...
              <div className="flex items-center gap-2 shrink-0">
                <span className={`text-[10px] font-mono uppercase px-2 py-0.5 rounded-sm ${style.color} ${style.bg}`}>
                  {style.label}
                </span>
                {isExpanded ? (
                  <ChevronUp className="w-4 h-4 text-graphite-500" strokeWidth={1.5} />
                ) : (
                  <ChevronDown className="w-4 h-4 text-graphite-500" strokeWidth={1.5} />
                )}
              </div>
            </button>
              ...
              <div className="px-3.5 pb-3.5 space-y-3 border-t border-graphite-800 pt-3">
                {isEditing ? (
                  <div className="space-y-2">
                    <div>
                      <label className="block text-[10px] uppercase tracking-wide text-graphite-500 font-medium mb-1">
                        Asunto
                      </label>
                      <input
                        type="text"
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        className="flex items-center gap-1.5 text-graphite-400 hover:text-graphite-200 text-xs px-3 py-1.5 transition-colors"
                      >
                  ...
                  <>
                    ...
                    <div className="flex items-center justify-between gap-2 flex-wrap">
                      <select
                        value={rfi.status}
                        onChange={(e) => handleStatusChange(rfi.id, e.target.value)}
                      ...
                      <div className="flex items-center gap-3">
                        <button
                          onClick={() => startEditing(rfi)}
                          className="flex items-center gap-1.5 text-xs text-graphite-400 hover:text-blueprint-400 transition-colors"
                        >
