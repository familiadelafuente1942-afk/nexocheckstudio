...
export default function EditProjectButton({ project }: { project: ProjectData }) {
  ...
  async function handleSave() {
    if (!name.trim()) {
      setError("El nombre de la obra no puede estar vacío.");
      return;
    }
  ...
  return (
    <>
      <button
        onClick={openModal}
        className="flex items-center gap-1.5 text-graphite-500 hover:text-blueprint-400 text-xs font-medium px-2.5 py-1.5 rounded-md hover:bg-graphite-800 transition-colors"
        title="Editar obra"
      >
          ...
          <div className="w-full max-w-md bg-graphite-900 border border-graphite-700 rounded-lg p-6">
            <div className="flex items-center justify-between mb-5">
              ...
              <button
                ...
                className="text-graphite-500 hover:text-graphite-200 p-1"
              >
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-3 pt-2">
                ...
                <button
                  onClick={() => setOpen(false)}
                  className="text-graphite-400 hover:text-graphite-200 text-sm px-4 py-2 transition-colors"
                >
                  Cancelar
                </button>
              </div>
