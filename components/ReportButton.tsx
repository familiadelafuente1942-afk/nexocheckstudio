...
export default function ReportButton({
  ...
  async function handleGenerate() {
    ...
    doc.text("NEXOCHECKSTUDIO", marginX, y);
    y += 14;
    ...
    doc.text(`${project.orgName}`, marginX, y);
    y += 6;
    if (project.clientName || project.location) {
      doc.text(
        `${project.clientName ?? "Sin cliente"}${project.location ? " · " + project.location : ""}`,
        marginX,
        y
      );
      y += 6;
    ...
    doc.text(`Documentos analizados: ${documents?.length ?? 0}`, marginX, y);
    y += 6;
    ...
    if (documents && documents.length > 0) {
      ...
      doc.setFontSize(13);
      doc.setTextColor(20, 20, 20);
      doc.text("Documentación", marginX, y);
      y += 8;
      ...
      for (const d of documents) {
        addPageIfNeeded(6);
        doc.text(`• ${d.name} — ${d.discipline} (Rev. ${d.revision ?? "A"})`, marginX, y);
        y += 5.5;
      }
      y += 8;
    }
    ...
    if (findings && findings.length > 0) {
      ...
      for (const f of findings) {
        ...
        doc.text(`[${SEVERITY_LABELS[f.severity] ?? f.severity}]`, marginX, y);

        doc.setFontSize(10);
        doc.setTextColor(20, 20, 20);
        doc.text(f.title, marginX + 22, y);
        y += 6;
    ...
    if (rfis && rfis.length > 0) {
      ...
      doc.setFontSize(13);
      doc.setTextColor(20, 20, 20);
      doc.text("RFIs recomendados", marginX, y);
      y += 8;
      ...
      for (const r of rfis) {
        addPageIfNeeded(6);
        doc.text(`${r.code ?? "RFI"} — ${r.subject} (${r.status})`, marginX, y);
        y += 5.5;
      }
    }
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.text(
        "Sistema inteligente de asistencia preventiva. No reemplaza al profesional responsable de la obra.",
        marginX,
        290
      );
    }
