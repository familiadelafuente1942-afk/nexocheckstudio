...
import {
  ...
  Sparkles, Layers, Calculator, X, ExternalLink, GitCompareArrows,
} from "lucide-react";
...
export default function DocumentsPanel({ projectId, organizationId }: { projectId: string; organizationId: string; }) {
  ...
  async function handleDisciplineChange(doc: DocumentRow, newDiscipline: string) {
    setUpdatingDisciplineId(doc.id);
    const { error: updateError } = await supabase
      .from("documents")
      .update({ discipline: newDiscipline })
      .eq("id", doc.id);

    if (!updateError) {
      setDocuments((prev) => prev.map((d) => (d.id === doc.id ? { ...d, discipline: newDiscipline } : d)));
    }
  ...
  async function handleCompare(doc: DocumentRow) {
    setComparingId(doc.id);
    setAnalyzeMessage(null);
    setError(null);
    try {
      const res = await fetch("/api/comparar-foto", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ documentId: doc.id }) });
      const data = await res.json();
      if (!res.ok) { setError(data.error || "No se pudo comparar la foto."); setComparingId(null); return; }
      setAnalyzeMessage(data.count > 0 ? `Comparación completa: ${data.count} observación(es) sobre "${doc.name}" vs. ${data.plansCompared} plano(s).` : `Comparación completa: la foto "${doc.name}" coincide razonablemente con los planos.`);
      setComparingId(null);
      window.location.reload();
    } catch { setError("Error de conexión al comparar la foto."); setComparingId(null); }
  }
      ...
      {loading ? (
        ...
        <div className="grid gap-2">
          {documents.map((doc) => {
            ...
            return (
              <div key={doc.id} className="bg-graphite-900 border border-graphite-700 rounded-lg p-3.5 flex items-center justify-between gap-3 flex-wrap">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-graphite-500 text-xs mt-0.5 font-mono">
                      Rev. {doc.revision ?? "A"} · {formatBytes(doc.file_size)}{docIsImage ? " · Foto" : ""}
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-1.5 shrink-0 ml-auto">
                  <select
                    ...
                    onChange={(e) => handleDisciplineChange(doc, e.target.value)}
                    disabled={updatingDisciplineId === doc.id}
                    className="bg-graphite-800 border border-graphite-600 rounded-md px-2 py-1.5 text-xs text-graphite-200 outline-none focus:border-blueprint-500 disabled:opacity-50"
                  >
                    {DISCIPLINAS.map((d) => <option key={d.value} value={d.value}>{d.label}</option>)}
                  </select>

                  {docIsImage ? (
                    <button onClick={() => handleCompare(doc)} disabled={comparingId === doc.id}
                      className="flex items-center gap-1 px-2 py-1.5 text-xs text-blueprint-400 hover:bg-graphite-800 rounded-md transition-colors disabled:opacity-50" title="Comparar foto con los planos">
                      {comparingId === doc.id ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <GitCompareArrows className="w-3.5 h-3.5" strokeWidth={1.5} />}
                      {comparingId === doc.id ? "Comparando..." : "Comparar con planos"}
                    </button>
                  ) : (
