...
export default function DocumentCompletenessPanel({ projectId }: { projectId: string }) {
  ...
  if (loading) {
    return <p className="text-graphite-500 text-xs">Cargando estado documental...</p>;
  }
  ...
  return (
    <div className="bg-graphite-900 border border-graphite-700 rounded-lg p-4">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-xs uppercase tracking-wide text-graphite-400 font-medium">
          Estado documental
        </h3>
        <span className="text-[11px] font-mono text-graphite-500">
          {presentCount} / {DISCIPLINAS_CHECKLIST.length} disciplinas
        </span>
      </div>
          ...
          return (
            <div
              ...
              className={`flex items-center gap-2 px-2.5 py-2 rounded-md text-xs ${
                present ? "text-graphite-300" : "text-graphite-500"
              }`}
            >
              {present ? (
                <CheckCircle2 className="w-3.5 h-3.5 text-signal-ok shrink-0" strokeWidth={1.5} />
              ) : (
                <XCircle className="w-3.5 h-3.5 text-graphite-600 shrink-0" strokeWidth={1.5} />
              )}
